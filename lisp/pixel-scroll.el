...
Target one row beyond MARGIN: a non-zero `vscroll' clips the boundary line, so
resting point exactly MARGIN rows from the edge can leave fewer than MARGIN
*whole* lines between point and the edge, which makes redisplay recenter.  The
extra row absorbs that fractional line.  Only point moves, never the window; do
nothing if point is already clear of both margins, or cannot be made compliant
within the current view (for example next to an image taller than the window)."
  (let ((row (cdr (posn-col-row (posn-at-point))))
        (slack (1+ margin)))
    (when row
      (let ((bottom (1- (window-text-height))))
        (cond ((< row slack)
               (vertical-motion (- slack row)))
              ((> row (- bottom slack))
               (vertical-motion (- (- bottom slack) row))))))))
        ...
        (let ((saved pixel-scroll-precision--saved-margin))
          (when (consp saved)
            (let ((margin (max 0 (min (cdr saved)
              ...
              (when (and pixel-scroll-precision-reposition-point (> margin 0))
